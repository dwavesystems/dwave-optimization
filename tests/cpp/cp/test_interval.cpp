...
TEST_CASE("Interval Integer (static size)") {
    GIVEN("A state manager and 10 intervals with default boundary") {
        std::unique_ptr<Copier> sm = std::make_unique<Copier>();
        IntIntervalArray interval = IntIntervalArray(sm.get(), 10);
        REQUIRE(interval.num_domains() == 10);
        REQUIRE(interval.max_size() == 10);
        REQUIRE(interval.min_size() == 10);
        ...
        for (size_t i = 0; i < interval.num_domains(); ++i) {
            REQUIRE(interval.min(i) == 0);
            REQUIRE(interval.max(i) == (ssize_t)1 << 51);
            REQUIRE(interval.size(i) == ((ssize_t)1 << 51) + 1);
        }
        ...
        AND_GIVEN("A test domain listener") {
            ...
            WHEN("We remove below 10.5 from all indices") {
                for (int i = 0; i < 10; ++i) {
                    auto status = interval.remove_below(10.5, i, tl_ptr);
                    REQUIRE(status == CPStatus::OK);
                }

                THEN("The minimum is correctly changed and the maximum is left unchanged") {
                    for (int i = 0; i < 10; ++i) {
                        REQUIRE(interval.min(i) == 11);
                        REQUIRE(interval.max(i) == (ssize_t)1 << 51);
                    }
                }

                AND_WHEN("We remove above 30.5 from the fourth index") {
                    auto status = interval.remove_above(30.3, 4, tl_ptr);
                    REQUIRE(status == CPStatus::OK);
                    REQUIRE(interval.max(4) == 30);
    ...
    GIVEN("An array of intervals initialized by two vectors") {
        std::unique_ptr<Copier> sm = std::make_unique<Copier>();
        std::vector<ssize_t> lb(10);
        std::vector<ssize_t> ub(10);
        ...
        IntIntervalArray interval2 = IntIntervalArray(sm.get(), lb, ub);
        REQUIRE(sm->store.size() == 22);
        THEN("The fomains are correctly initialized") {
            REQUIRE(interval2.num_domains() == lb.size());
            REQUIRE(interval2.min_size() == 10);
            ...
            for (int i = 0; i < 10; ++i) {
                REQUIRE(interval2.min(i) == -i);
                REQUIRE(interval2.max(i) == i);
                REQUIRE(interval2.size(i) == 2 * i + 1);
            }
        }
    }

    GIVEN("An array of 15 intervals between 1 and 5") {
        std::unique_ptr<Copier> sm = std::make_unique<Copier>();
        IntIntervalArray interval3 = IntIntervalArray(sm.get(), 15, 1, 5);
        REQUIRE(sm->store.size() == 32);
        THEN("The domains are correctly set") {
            REQUIRE(interval3.num_domains() == 15);
            REQUIRE(interval3.min_size() == 15);
            REQUIRE(interval3.max_size() == 15);
            for (int i = 0; i < 15; ++i) {
                REQUIRE(interval3.min(i) == 1);
                REQUIRE(interval3.max(i) == 5);
                REQUIRE(interval3.size(i) == 5);
            }
        }
    }
}

TEST_CASE("Interval integer (dynamic size)") {
    GIVEN("A state manager and a dynamic array with minimum size 6 and max size 10") {
        std::unique_ptr<Copier> sm = std::make_unique<Copier>();
        IntIntervalArray interval = IntIntervalArray(sm.get(), 6, 10);
        REQUIRE(interval.num_domains() == 10);
        REQUIRE(interval.min_size() == 6);
        REQUIRE(interval.max_size() == 10);
        // store size should be 2 x 10 + 2 for min and max
        ...
        AND_GIVEN("A test domain listener") {
            ...
            WHEN("We remove below 10.5 from all indices") {
                ...
                AND_WHEN("We remove above 31.6 from the fourth index") {
                    auto status = interval.remove_above(31.6, 4, tl_ptr);
                    REQUIRE(status == CPStatus::OK);
                    REQUIRE(interval.max(4) == 31);
    ...
    GIVEN("An array of intervals initialized by two vectors") {
        ...
        IntIntervalArray interval2 = IntIntervalArray(sm.get(), 6, lb, ub);
        REQUIRE(sm->store.size() == 22);
        THEN("The domains are correctly initialized") {
            REQUIRE(interval2.num_domains() == lb.size());
            REQUIRE(interval2.min_size() == 6);
    ...
    GIVEN("An array of 15 intervals between 1 and 5") {
        std::unique_ptr<Copier> sm = std::make_unique<Copier>();
        IntIntervalArray interval3 = IntIntervalArray(sm.get(), 6, 15, 1, 5);
        REQUIRE(sm->store.size() == 32);
        THEN("The domains are correctly set") {
            REQUIRE(interval3.num_domains() == 15);
            REQUIRE(interval3.min_size() == 6);
