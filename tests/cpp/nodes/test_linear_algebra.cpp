...
TEST_CASE("MatMulNode") {
    auto graph = Graph();

    GIVEN("A dynamic testing array node and matmul on it") {
        ...
        auto matmul = MatMulNode(&arr, &add);
        THEN("MatMulNode reports correct min and max") {
    ...
    GIVEN("Two constant 1d nodes and a MatMulNode") {
        ...
        auto matmul_ptr = graph.emplace_node<MatMulNode>(c1_ptr, c2_ptr);
        ...
        WHEN("We initialize a state") {
            ...
            THEN("The initial MatMulNode state is correct") {
                CHECK_THAT(matmul_ptr->view(state), RangeEquals({32}));
            }
        }
    }

    GIVEN("Two constant 2d nodes and a MatMulNode") {
    ...
    GIVEN("Two constant 2d nodes and a MatMulNode") {
        ...
        auto matmul_ptr = graph.emplace_node<MatMulNode>(c2_ptr, c1_ptr);
    ...
    GIVEN("Two 4d constant nodes and a MatMulNode") {
        std::vector<ssize_t> c1_shape{5, 3, 1, 7};
        ssize_t c1_size = 5 * 3 * 1 * 7;
    ...
    GIVEN("A set node and MatMulNode representing self dot product") {
        auto set_ptr = graph.emplace_node<SetNode>(10);
        auto matmul_ptr = graph.emplace_node<MatMulNode>(set_ptr, set_ptr);
        graph.emplace_node<ArrayValidationNode>(matmul_ptr);
    ...
    GIVEN("A 2d dynamic testing node and a 1d constant") {
        ...
        CHECK_THROWS_AS(MatMulNode(c_ptr, arr_ptr), std::invalid_argument);

        auto matmul_ptr = graph.emplace_node<MatMulNode>(arr_ptr, c_ptr);
        graph.emplace_node<ArrayValidationNode>(matmul_ptr);
    ...
    GIVEN("A 2d dynamic testing node and a 1d slice") {
        ...
        CHECK_THROWS_AS(MatMulNode(arr_ptr, vec_ptr), std::invalid_argument);

        auto matmul_ptr = graph.emplace_node<MatMulNode>(vec_ptr, arr_ptr);
        graph.emplace_node<ArrayValidationNode>(matmul_ptr);
