...
TEST_CASE("ConstantNode") {
    ...
    GIVEN("A constant node that is a view of data held by a vector") {
        std::vector<double> values = {30, 10, 40, 20, 50};  // deliberately too large
        auto ptr = graph.emplace_node<ConstantNode>(
                values.data(), std::initializer_list<ssize_t>{2, 2}
        );
    ...
    SECTION("exception - dynamic shape") {
        ...
        REQUIRE_THROWS_AS(
                graph.emplace_node<ConstantNode>(
                        values.data(), std::initializer_list<ssize_t>{-1, 2}
                ),
    ...
    SECTION("exception - dynamic, size 0 shape") {
        ...
        REQUIRE_THROWS_AS(
                graph.emplace_node<ConstantNode>(
                        values.data(), std::initializer_list<ssize_t>{-1, 0}
                ),
                std::invalid_argument
        );
    }
