...
TEST_CASE("BasicIndexingNode") {
    SECTION("SizeInfo") {
        ...
        SECTION("Dynamic(3, 5, 2)[5:7]") {
            auto dynamic = DynamicArrayTestingNode(std::initializer_list<ssize_t>{-1, 5, 3});

            auto slice = BasicIndexingNode(&dynamic, Slice(5, 7), Slice(1, std::nullopt), Slice());

            // Haven't thought this one through fully yet
            auto sizeinfo = slice.sizeinfo();
            CHECK(sizeinfo.array_ptr == &dynamic);
            CHECK(sizeinfo.multiplier == fraction(4, 5));
            CHECK(sizeinfo.offset == -(5 * 4 * 3));
            CHECK(!sizeinfo.min.has_value());
            CHECK(sizeinfo.max == 2 * 4 * 3);
