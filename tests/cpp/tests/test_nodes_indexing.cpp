...
TEST_CASE("LenNode") {
    GIVEN("A 0d node") {
        ...
        WHEN("We create a node accessing the length") {
            ...
            THEN("The LenNode is a scalar output") {
                CHECK(len.size() == 1);
                CHECK(len.ndim() == 0);
                CHECK(!len.dynamic());
            }
    ...
    GIVEN("A graph with a SetNode and a corresponding LenNode, and a state") {
        auto graph = Graph();

        auto s_ptr = graph.emplace_node<SetNode>(5);
        auto len_ptr = graph.emplace_node<LenNode>(s_ptr);

        auto state = graph.empty_state();
        s_ptr->initialize_state(state, {});  // default to empty
        graph.initialize_state(state);

        THEN("The state of the LenNode is the same as the size of the SetNode") {
