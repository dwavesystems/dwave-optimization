...
class NumberNodeStateData : public ArrayNodeStateData, public CheckpointableState {
 public:
    ...
    NumberNodeStateData(const NumberNode& node, std::vector<double> input) :
        ArrayNodeStateData(std::move(input)), sum_constraints_lhs(), slice_cache_(), node_(node) {}
...
void NumberNodeStateData::update_(
    ...
    if (optional_slices) {
        slices = std::move(*optional_slices);
    } else {
        slices.reserve(num_sum_constraints);
        for (ssize_t i = 0; i < num_sum_constraints; ++i) {
            const auto axis = sum_constraints[i].axis();
            // *NOTE*: If `axis` is defined, get appropriate slice. Otherwise,
            // array is treated as a flat array with one slice.
            slices.emplace_back((axis) ? unravel_index(index, node_.shape())[*axis] : 0);
        }
    }
    assert(sum_constraints.size() == slices.size());
    assert([&] {  // Check `slices`. See *NOTE*.
        for (ssize_t i = 0; i < num_sum_constraints; ++i) {
            const auto axis = sum_constraints[i].axis();
            if (slices[i] != (axis ? unravel_index(index, node_.shape())[*axis] : 0)) return false;
        }
        return true;
    }());

    for (ssize_t i = 0; i < num_sum_constraints; ++i) {
        const ssize_t slice = slices[i];
        sum_constraints_lhs[i][slice] += difference;  // Update slice sum
    }
...
std::vector<std::vector<double>> get_sum_constraints_lhs(
    ...
    for (
        BufferIterator<const double, const double> it(
            number_data.data(), node_shape, node.strides()
        );
...
void construct_state_given_exactly_one_sum_constraint(
    ...
    const BufferIterator<double, double> slice_0_it(
...
BinaryNode::BinaryNode(
    ...
    BinaryNode({size}, std::move(lower_bound), std::move(upper_bound), std::move(sum_constraints)) {
}
...
void BinaryNodeStateData::update_(
    ...
    for (ssize_t i = 0; i < num_sum_constraints; ++i) {
        // Update slice sum and tracked indices.
        const ssize_t slice = slices[i];
        sum_constraints_lhs[i][slice] += difference;
        if (difference == 1.0) {
