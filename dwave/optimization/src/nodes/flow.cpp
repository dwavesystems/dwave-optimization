...
void ExtractNode::propagate(State& state) const {
    ...
    if (condition_ptr_->diff(state).size() == 0 && arr_ptr_->diff(state).size() == 0) return;
    ...
    auto add_true = [](ssize_t acc, double val) -> ssize_t { return acc + (bool)val; };
    ssize_t count =
    ...
    node_data->assign_partial(new_values, count, true);
}
