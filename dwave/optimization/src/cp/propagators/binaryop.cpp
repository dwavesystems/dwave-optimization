...
CPStatus BinaryOpPropagator<std::plus<double>>::propagate(CPPropagatorsState& p_state,
    ...
    while (data->num_indices_to_process() > 0) {
        ...
        // prune the output
        status = out_->remove_above(v_state, max_i, i);
        if (status == CPStatus::Inconsistency) return status;

        status = out_->remove_below(v_state, min_i, i);
        if (status == CPStatus::Inconsistency) return status;
        ...
        status = lhs_->remove_above(v_state, lhs_up, i);
        status = lhs_->remove_below(v_state, lhs_lo, i);
        ...
        status = rhs_->remove_above(v_state, rhs_up, i);
        if (status == CPStatus::Inconsistency) return status;

        status = rhs_->remove_below(v_state, rhs_lo, i);
        if (status == CPStatus::Inconsistency) return status;
    }
    return status;
}
...
CPStatus BinaryOpPropagator<std::less_equal<double>>::propagate(CPPropagatorsState& p_state,
    ...
    while (data->num_indices_to_process() > 0) {
        ...
        if (out_->min(v_state, i) == 1) {
            ...
            status = lhs_->remove_above(v_state, rhs_->max(v_state, i), i);
            if (status == CPStatus::Inconsistency) return status;

            status = rhs_->remove_below(v_state, lhs_->min(v_state, i), i);
            if (status == CPStatus::Inconsistency) return status;
            ...
            status = lhs_->remove_below(v_state, rhs_->min(v_state, i), i);
            if (status == CPStatus::Inconsistency) return status;

            status = rhs_->remove_above(v_state, lhs_->max(v_state, i), i);
            if (status == CPStatus::Inconsistency) return status;
